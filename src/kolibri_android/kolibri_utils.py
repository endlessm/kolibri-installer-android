...
def init_kolibri(content_fallback_dirs=None, **kwargs):
    logging.info("Initializing Kolibri and running any upgrade routines")

    _monkeypatch_whitenoise()
    _init_kolibri_env(content_fallback_dirs)
...
def _monkeypatch_whitenoise():
    from .android_whitenoise import monkeypatch_whitenoise

    monkeypatch_whitenoise()


def _init_kolibri_env(content_fallback_dirs=None):
    ...
    if endless_key_uris is not None:
        content_fallback_dirs = DynamicWhiteNoise.encode_root(
            endless_key_uris["content"]
        )
        logging.info(
            "Setting KOLIBRI_CONTENT_FALLBACK_DIRS to %s", content_fallback_dirs
        )
